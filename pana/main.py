...
# UI theme objects use wrapper function references so _theme.apply_theme()
...
class MiniApp:
    ...
    async def _stream_response(self, user_text: str) -> None:
        ...
        def on_abort() -> None:
            ...
            Sets cancel_event so the stream exits at the next token boundary,
            then immediately restores the UI. Messages submitted during the
            drain are queued and replayed once the finally block calls
        ...
        def event_handler(event) -> None:
            ...
            # Keep loader pinned to the bottom.
            self._chat_container.remove_child(loader)
        ...
        try:
            ...
            # task.cancel() from asyncio.run() teardown; on_abort was not called.
            ...
            if cancel_event.is_set():
                # on_abort already restored the editor; clear the draining flag.
    ...
    async def _cmd_login(self) -> None:
        ...
        async def on_select(item: SelectItem) -> None:
            ...
            try:
                ...
                self._add_message(
                    Text(_theme.success(f"Authenticated with {item.value}."), padding_x=1, padding_y=0)
                )
            except Exception as e:
