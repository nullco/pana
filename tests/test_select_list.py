...
async def test_wraps_up_from_first_item_to_last() -> None:
...
async def test_page_up_moves_by_max_visible_items() -> None:
    ...
    await sl.handle_input(_PAGE_UP)
    assert sl._selected_index == 7


async def test_page_down_moves_by_max_visible_items() -> None:
    """Page down must move the selection down by max_visible items."""
    items = [SelectItem(value=str(i), label=f"item-{i}") for i in range(20)]
    sl = _make_list(items, max_visible=5)
    sl.set_selected_index(3)
    await sl.handle_input(_PAGE_DOWN)
...
async def test_searchable_filters_by_typing() -> None:
    ...
    sl = SelectList(items, 10, _THEME, searchable=True)
    await sl.handle_input("b")
    assert len(sl._filtered) == 1
    assert sl._filtered[0].value == "beta"


async def test_searchable_backspace_widens_filter() -> None:
...
async def test_non_searchable_ignores_text_input() -> None:
